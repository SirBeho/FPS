...
public class PlayerMovement : MonoBehaviour
{
    ...
    public GameObject pausa_;

    void Start()
    {
        jump = new InputAction("Jump", binding: "<keyboard>/space");
        pausas = new InputAction("Pausas", binding: "<keyboard>/escape");
    ...
    void Update()
    {
        ...
        if (Mathf.Approximately(pausas.ReadValue<float>(), 1))
        {
            Debug.Log(pausas.ToString());
            Cursor.lockState = CursorLockMode.None;
            pausa_.SetActive(true);
            Time.timeScale = 0;
            return;
        }
        ...
        animator.SetFloat("velocidad", Mathf.Abs(x) + Mathf.Abs(z));
